...
Input Format:
The first line of input contains an integer, that denotes the value of total number of people standing in queue 
or the size of the array of priorities. Let us denote it with the symbol N.
...
Next, the person with priority 9 comes out. 
And there is no person with higher priority than him. So he'll get the ticket. Queue's status :  {4, 3}. Time : 1 secs.
...
public class Buytheticket {
	...
	public static int buyTicket(int[] input, int k) {
	    ...
	    // Create a queue to store the indices of people in the queue
	    ...
	    while (!queue.isEmpty()) {
	        ...
	        // Check if there is any person with higher priority than the front person
	        boolean hasHigherPriority = false;
	        for (int index : queue) {
	            if (input[index] > input[frontIndex]) {
	                hasHigherPriority = true;
	                break;
	            }
	        }

	        // If the removed person is you (at index k), return the current value of time
	        if (frontIndex == k && !hasHigherPriority) {
	            return time + 1;
	        }

	        // If there is a person with higher priority, move the front person to the end of the queue
